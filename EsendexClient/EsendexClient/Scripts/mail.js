(function () {
    ...
    app.controller("MailboxController", function($scope, $http) {
        ...
        var addInboundMessage = function(message) {
        ...
        var updateFailedMessage = function(message) {
        ...
        var updateDeliveredMessage = function(message) {
            if ($scope.activeConversation !== undefined) {
                for (var i = 0; i < $scope.activeConversation.length; ++i) {
                    if ($scope.activeConversation[i].id == message.messageId) {
                        $scope.activeConversation[i].lastStatusAt = formatDate(new Date(message.occurredAt));
        ...
        $scope.conversationHub.client.onInboundMessage = addInboundMessage;
        $scope.conversationHub.client.onMessageFailed = updateFailedMessage;
        $scope.conversationHub.client.onMessageDelivered = updateDeliveredMessage;

        $scope.accountEventHandlerHub = $.connection.accountEventHandlerHub;
        $scope.accountEventHandlerHub.client.onInboundMessage = addInboundMessage;
        $scope.accountEventHandlerHub.client.onMessageFailed = updateFailedMessage;
        $scope.accountEventHandlerHub.client.onMessageDelivered = updateDeliveredMessage;
        
        $scope.registerRestPushNotifications = function () {
            $scope.pushType = "rest";
            $scope.accountEventHandlerHub.server.unregister();
            $scope.conversationHub.server.register($scope.account.id);
        };

        $scope.registerAccountEventHandlerPushNotifications = function () {
            $scope.pushType = "formPost";
            $scope.conversationHub.server.unregister();
            $scope.accountEventHandlerHub.server.register($scope.account.reference);
        };
        ...
        $scope.startPushNotifications = function () {
            $http.get(apiControllers[$scope.apiType].account).success(function (account) {
                ...
                $.connection.hub.start().done(function() {
                    $scope.registerRestPushNotifications();
                });
            });
        };
